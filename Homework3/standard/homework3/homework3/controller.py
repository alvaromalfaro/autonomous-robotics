...
class Controller(Node):
    ...
    def __init__(self):
        ...
        self.add_turtle("turtle1", 5.5 - self.radius, 5.5 + self.radius +
    ...
    def go_curve(self, final_rotate=False):
        ...
        if final_rotate:
            self._action_client = ActionClient(
                self, RotateAbsolute, '/GUIDANCE/rotate_absolute')
    ...
    def listener_callback(self, msg):
        ...
        if self.iteration % 10 == 0:
            self.get_logger().info(
                f'Average error {self.agg_error/float(self.iteration)}')
