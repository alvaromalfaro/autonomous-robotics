...
appropriate angular velocity ($\omega_z$) to minimize the \textbf{cross-track error} (CTE) relative
to the guidance path.
...
\subsection{Control loop discretization and frequency}

The base implementation force the \texttt{listener\_callback} to execute only once per second. This
limitation was reduces to $0.1$ seconds, allowing the robot to correct its orientation more
frequently. The derivative term is computed using the actual elapsed time between callbacks:
\begin{equation}
    \frac{d cte_t}{dt} \approx \frac{cte_t - cte_{t-1}}{\Delta t}
\end{equation}
This ensures that the force applied by the $\tau_d$ term scales relatively to the physical time step,
preventing the controller from becoming unstable at higher frequencies.
...
Following the initial lap, the descent algorithm activated. The Table \ref{tab:optimization} illustrates
...
\begin{table}
    ...
    \begin{tabular}{lccc}
        \hline
        \textbf{Lap} & \textbf{Average CTE ($|cte_t|$)} & \textbf{Proportional Gain ($\tau_p$)} & \textbf{Derivative Gain ($\tau_d$)} \\
        \hline
...
As observed in Figure \ref{fig:convergence}, the Twiddle algorithm achieved a near-monotonic
convergence, reducing the average CTE from an initial baseline of approximately $0.17$ down to under
$0.08$ by the final lap.
...
algorithm also increased $\tau_d$ to provide additional damping. This coupled stair-step growth
allowed the controller to track the reference trajectory while mitigating overshoot in curved
sections. Furthermore, the lack of downward adjustments in the parameter values indicates a smooth
descent towards a local minimum without being destabilized.

\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{img/twiddle_results.png}
